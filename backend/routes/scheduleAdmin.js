...
router.get('/list_schedules', async (req, res) => {
  try {
...
router.get('/schedule_detail', async (req, res) => {
  try {
...
router.get('/live_sessions', async (req, res) => {
  try {
...
router.get('/session_students', async (req, res) => {
  try {
...
router.get('/conflict_check', async (req, res) => {
  try {
...
router.post('/update_schedule', async (req, res) => {
  try {
...
router.post('/boot_user', async (req, res) => {
  try {
...
router.get('/pending_instructors', async (req, res) => {
  try {
  ...
  `, [like]);
    ...
    res.json({ success: true, instructors: result.rows, pending_count: parseInt(countRes.rows[0].count) });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});

router.post('/approve_instructor', async (req, res) => {
  try {
    ...
    if (!userCheck.rows.length) return res.status(404).json({ success: false, message: 'Pending instructor not found.' });
    ...
    if (cuid) {
      ...
      if (dupCheck.rows.length) return res.status(409).json({ success: false, message: `UID '${cuid}' is already assigned.` });
    }
    ...
    `, [user_id, code, `${user.first_name} ${user.last_name}`, 'College of Teacher Education', user.email, user.contact_no, cuid || null]);
    ...
    auditLog(req.session.userId, 'approve_instructor', 'instructors', user_id, { card_uid: cuid }, req.ip);
...
router.post('/assign_instructor_rfid', async (req, res) => {
  try {
...
router.get('/instructor_detail', async (req, res) => {
  try {
